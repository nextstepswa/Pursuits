...
## Non-terminated active pursuit fatalities only
pre.post.apf.noterms <- fatalities.p %>%
  filter(date > date.sameperiod.before & grepl("Active", vpursuit)) %>%
  group_by(postreform) %>%
  summarize(Subjects = sum(victim == "Subject"),
            Bystanders = sum(victim == "Bystander"),
            Passengers = sum(victim == "Passenger"),
...
pct.change.post.reform.noterms <- 
  (pre.post.apf.noterms$Total[pre.post.apf.noterms$postreform==1]/
  pre.post.apf.noterms$Total[pre.post.apf.noterms$postreform==0]) - 1


## active pursuits + attempted stops
pre.post.apf.wstops <- fatalities.all %>%
  filter(date > date.sameperiod.before & grepl("Active|Attempt|Terminated", vpursuit)) %>%
  group_by(postreform) %>%
  summarize(Subjects = sum(victim == "Subject"),
            Bystanders = sum(victim == "Bystander"),
            Passengers = sum(victim == "Passenger"),
...
pct.change.post.reform.wstops <- 
  (pre.post.apf.wstops$Total[pre.post.apf.wstops$postreform==1]/
  pre.post.apf.wstops$Total[pre.post.apf.wstops$postreform==0]) - 1
...
save.image(file = here::here("data-outputs", "WApursuits.rda"))

#load(file = here::here("data-outputs", "WApursuits.rda"))
