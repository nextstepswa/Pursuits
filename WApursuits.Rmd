...
if(file.exists(here::here("data-outputs", "FEpursuits.rda"))){
  ...
  if(lastmod.wa2015 > lastmod.pursuit | lastmod.makeFE > lastmod.pursuit) {
    ...
    print(message("Rebuilding pursuit dataset"))
...
* "After pursuit fatalities" -- a pursuit was involved earlier in the incident, but had ended and the subject was subsequently killed (typically by gunshot).
...
* *Calendar year:*  This does not give a very clear picture of the impact, because the law was changed in the middle of 2021.  The first clear comparisons will be between 2022 and later years to 2020 and earlier years.  At this point we only have a single year to compare in the post-reform period.
...
The Fatal Encounters dataset is not perfect.  They also likely undercount the true number of fatalities associated with pursuits, as the search methods used by this project rely on these cases leaving a digital signature online, and not all pursuit fatalities will be reported this way.  They do, however, provide more complete information, on a wider range of cases, and in a more timely manner, than the official data.  
...
* *Involved pursuit * -- a vehicle pursuit occurred during the incident, but it had finished before the person was killed, and the vehicles involved in an active or just terminated pursuit were not the cause of death. 
...
In 1 incident, the events lay close to the boundary between "pursuit-involved" and "terminated pursuit".  This incident involved a speeding motorist who was pursued multiple times over the course of an hour by 4 different state patrol troopers.  In each case, the pursuit was terminated for safety reasons, as the motorist exceeded 100 mph, weaving in and out of traffic.  The last pursuit terminated 10 minutes before the motorist crashed into another vehicle, killing 2 people in that vehicle.  Since the pursuit was terminated 10 minutes before the crash, and the terminated pursuit classification requires termination within a minute or a mile of the vehicular homicide, we classified this incident as "pursuit-involved".
____
